...
\maketitle
\begin{abstract}
We study uncolored multisets $F$ of positive integers with maximum $n$
...
\]
All multisets in this paper are uncolored: the two available copies of a
...
Specializing the counting formula of Chu et al.\
...
\section{The specialized constructions}
...
$A_n=\A_2(n)$. We now realize \eqref{eq:positive} by explicit multiset maps, using
...
Related colored families occur in \cite{CIMSZ}. Their distinction from
the present multisets is already visible at small indices. At maximum three, the two-colored family satisfying
$\min C\ge|C|$ has members $\{3\},\{2_1,3\},\{2_2,3\}$, whereas
$B_3$ has members $\{3\},\{2,3\},\{2,2,3\}$. Forgetting colors
identifies two members and misses the double occurrence. Thus equality of these counts does not arise by simply forgetting colors.
...
\cite{Mathlib}. The development also verifies the finite residue partition,
...
above but are outside the formalization. The supplementary material
...
the Lean formalizations, and drafting and editing the manuscript.
